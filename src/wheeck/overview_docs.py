...
from common.constants import (OVERVIEW_DEFAULT_FONT, OVERVIEW_FORMATS, PATH_CFG_PRJ, PATH_LOG, PATH_RES,
...
def generate_overview(year: int = date.today().year, month: int = date.today().month) -> None:
    ...
    if querier.run(QUERY_GET_OVERVIEW, year, month).rows:
        ...
        for ws in [wb['cifre'], wb['litri'], wb['cifre manuale'], wb['litri manuale']]:
            ...
            for row_num, cell in enumerate([day for day in Calendar().itermonthdates(year, month) if day.month == month], start=3):
