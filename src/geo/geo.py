...
from common.constants import PATH_CFG_PRJ, PATH_LOG
...
class GeoMap:
    ...
    def __init__(self) -> None:
        ...
        """
        with open(PATH_CFG_PRJ, encoding='utf-8') as jin:
    ...
    def search(self, address: str, country: str = 'IT') -> tuple[float, float] | None:
        ...
        try:
            ...
            logger.error('API error on searching (%s, %s)! [status code: %d, error code: %s, error message: %s]',
                         address, country, e.status, e.message['error']['code'], e.message['error']['message'])
        except exceptions.HTTPError as e:
            logger.error('HTTP error on searching (%s, %s)! [status code: %d]', address, country, e.status_code)
    ...
    def get_distance_from_coords(self, departure: tuple[float, float], destination: tuple[float, float]) -> float | None:
        """
        ...
        try:
            ...
            logger.error('API error on calculating distance (%s, %s)! [status code: %d, error code: %s, error message: %s]',
                         departure, destination, e.status, e.message['error']['code'], e.message['error']['message'])
        except exceptions.HTTPError as e:
            logger.error('HTTP error on calculating distance (%s, %s)! [status code: %d]', departure, destination, e.status_code)
        except exceptions.Timeout:
