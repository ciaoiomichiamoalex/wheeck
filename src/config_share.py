...
PATH_CFG_COMMON = f'{PATH_CFG}/common.json'
PATH_LOG = f"{PATH_PRJ}/log/wheeck_{date.today().strftime('%Y_%m_%d')}.log"
...
QUERY_GET_LAST_OVERVIEWS = """
    SELECT DISTINCT EXTRACT(YEAR FROM delivery_date) AS year,
        EXTRACT(MONTH FROM delivery_date) AS month
    FROM wheeck.delivery 
    WHERE recording_date::DATE = NOW()::DATE
    ORDER BY 1, 2;
"""
QUERY_GET_SUMMARY = """
    WITH year_summary AS (
        SELECT document_number,
            ...
            vehicle,
            ROW_NUMBER() OVER (
                PARTITION BY document_date, vehicle 
                ORDER BY distance DESC
            ) AS rnk
        FROM wheeck.delivery
        WHERE EXTRACT(YEAR FROM document_date) = ?
